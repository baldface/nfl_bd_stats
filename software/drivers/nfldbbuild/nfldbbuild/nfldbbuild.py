...
class NflDatabaseBuild(object):
    ...
    def get_week_raw_json_data(self, season: int, game_type: str, week: int):
        ...
        if bool(list(week_root_child)):
            ...
            raise ValueError("No game data available. Verify the season ({season}), game type ({game_type}) and week "
                             "({week}) parameters provided are valid"
                             .format(season=season, game_type=game_type, week=week))
